...
To use the Check Runs API, the GitHub App must have the `checks:write` permission and can also subscribe to the [check_run]（There's a problem here，“Check_run” there are two C's when translating into Chinese documents）(/webhooks/event-payloads#check_run) webhook.
